...
import { clearStatFile, readCommand, readStatFile, sendCommand } from './sendHyperCommands'
...
// - FIREBASE AUTH CODE
import { getThemeByIndex } from '@features/common/utils/theme'
...
async function autoUpdate() {
    ...
    try {
        ...
        console.log('auto update failed with: ', error)
    }
}
...
const createWindow = () => {
    ...
    ipcMain.on('openEmulatorFolder', async () => {
        const externalAppPath = path.join(app.getAppPath(), 'emu', 'hyper-screw-fbneo', 'roms')
        try {
            ...
            console.log('Error opening emulator folder:', error)
        }
    })

    const setEmulatorPath = async (isResetPath = false) => {
        // the below path is only for the default emulator settings
        const externalAppPath = path.join(app.getAppPath(), 'emu', 'hyper-screw-fbneo')
        if (isResetPath) {
            const filePath = path.join(filePathBase, 'config.txt')
            mainWindow.webContents.send('message-from-main', `reset file path`)
   ...
   async function getEmulatorPath() {
        await getConfigData()
    ...
    const setAppTheme = async (themeIndex: number) => {
        ...
        try {
            ...
            //console.log(`CONFIG: Updated theme to ${getThemeByIndex(themeIndex).name}`)
        } catch (error) {
            mainWindow.webContents.send('message-from-main', error)
            console.error('Failed to write to config file:', error)
        }
    ...
    function saveRefreshToken(refreshToken: string) {
        fs.writeFileSync(tokenFilePath, JSON.stringify({ refreshToken }), {
            encoding: 'utf-8',
        })
    }
    ...
    ipcMain.on('startGameOnline', async (event, data) => {
        ...
        if (socket && emuListener) {
            ...
            function sendMessageToS(kill: boolean) {
                ...
                try {
                    socket.send(message, 0, message.length, serverPort, serverHost, function (err) {
