...
import { CloseButton } from '@features/common/ui/close-button'
...
import { useLayoutStore, useLoginStore, useMessageStore } from '@features/common/state'
import {
    RegExpMatcher,
...
export default function LobbiesPanel({ onLobbyChange }) {
    ...
    const handleLobbySelection = (lobby) => {
        if (lobby.isPrivate) {
            setOpen(true)
                ...
                {currentLobbiesState.map((lobby, index) => {
                    return (
                        <Button
                            ...
                            onClick={() => handleLobbySelection(lobby)}
                        >
