...
impl ProxyRuntime {
  async fn new(app: AppHandle, args: StartArgs) -> anyhow::Result<Arc<Self>> {
    ...
    let emu_listener = match UdpSocket::bind(SocketAddr::from((Ipv4Addr::LOCALHOST, emu_port))).await {
      Ok(s) => s,
      Err(_e) => {
  ...
  async fn spawn_local_reader(self: &Arc<Self>, mut stop_rx: oneshot::Receiver<()>) -> anyhow::Result<()> {
...
async fn spawn_emulator_reader(self: &Arc<Self>) -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            match emu_listener.recv_from(&mut buf).await {
                ...
                Err(_e) => {
                    let _ = this.app.emit_to(                // ✅ use `this`
                        EventTarget::any(), "proxy-log", "emu recv error".to_string()
                    );
                    break;
                }
            }
  ...
  async fn start_emulator(&self) -> anyhow::Result<()> {
    ...
    // ).ok();
    let _ = self.app.emit_to(EventTarget::any(), "proxy-log", format!("Port busy"));
    ...
    cmd.args([
      "--local-port", &emu_game_port.to_string(),
      "--remote-ip", "127.0.0.1",
      "--remote-port", &emu_listen_port.to_string(),
      "--player", &self.args.player.to_string(),
      "--delay", &self.args.delay.to_string(),
...
#[tauri::command]
pub async fn start_proxy(app: AppHandle, state: tauri::State<'_, ProxyManager>, args: StartArgs) -> Result<String, String> {
  let rt = ProxyRuntime::new(app, args).await.map_err(|e| e.to_string())?;
  rt.start().await.map_err(|e| e.to_string())?;
  *state.inner.lock().await = Some(rt.clone());
  Ok(format!("proxy started: local={} emu_listener={}",
